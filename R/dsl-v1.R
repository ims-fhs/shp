...
open <- function(path) {
  ...
  assertthat::assert_that(dir.exists(paste0(path, "/SHP-Data-W1-W21-SPSS")))
  assertthat::assert_that(dir.exists(paste0(path, "/SHP-Data-W1-W21-SPSS/W1_1999")))
...
longitudinal <- function(data, var_name, path. = path) {
  # browser()
  available <- years_available("P_USER.sav", paste0(path., "/SHP-Data-W1-W21-SPSS"), var_name)
  ...
  data <- import_long_cols("P_USER.sav", paste0(path., "/SHP-Data-W1-W21-SPSS"), cols = c("IDPERS", var_name), year_start = as.character(min(years)), year_end = as.character(max(years)))
  return(data)
}
...
cross_sectional <- function(data, var_name, path. = path) {
  path. <- paste0(path., "/SHP-Data-W1-W21-SPSS")
