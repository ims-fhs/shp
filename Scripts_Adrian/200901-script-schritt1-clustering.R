...
# data <- import_long_cols("P_USER.sav", "data/rawdata/Data SPSS/SHP-Data-W1-W21-SPSS", cols = c("IDPERS", "OCCUPAXX", "AGEXX"), year_start = "2008", year_end = "2017")
# occupied <- data %>%
...
# data <- import_long_cols("P_USER.sav", "data/rawdata/Data SPSS/SHP-Data-W1-W21-SPSS", cols = c("IDPERS", "PXXC17"), year_start = "2008", year_end = "2017")
# depression <- data %>%
#   select(!starts_with("AGE")) %>%
#   pivot_longer(cols = starts_with("OCCUPA"), names_to = "year", names_prefix = "OCCUPA", values_to = "occupation") %>%
#   group_by(ID) %>%
